...
Features carried over from bluesky_unified_threaded_fixed:
- post_message / post_with_image / post_video / post_reply
- update_display_name / update_profile_picture / update_banner / update_all_profile
- get_author_posts / get_friend_posts / get_timeline / get_profile_info

New in SkyWave:
- get_user_stats(handle=None)  – followers, following, posts count, etc.
- update_description(text)     – update profile description / bio

TouchDesigner should consume status updates and returned data
through the bridge script (touchdesigner_bluesky_bridge.py).
"""
...
class SkyWaveThreaded:
    ...
    def _feed_item_to_row(self, item: Any, fallback_type: str = "post", username: str = "") -> Optional[dict]:
        try:
            ...
            author_avatar = getattr(author_obj, "avatar", "") or ""
    ...
    def _do_update_description(self, new_description: str) -> None:
        client, username = self._login_client()
        self.push_status("UPDATING", f"Setting description: {new_description[:80]}")
        ...
        updated_profile = client.get_profile(username)
        self.push_status("SUCCESS", f"Description updated: {updated_profile.description[:80] if updated_profile.description else ''}")
    ...
    def _do_get_user_stats(self, handle: Optional[str]) -> None:
        client, username = self._login_client()
        target_handle = (handle or username).strip()
        self.push_status("LOADING", f"Fetching stats for @{target_handle}...")

        profile = client.get_profile(target_handle)

        stats = {
            "handle": getattr(profile, "handle", target_handle),
            "display_name": getattr(profile, "display_name", "") or "",
            "description": getattr(profile, "description", "") or "",
            "followers_count": str(getattr(profile, "followers_count", 0) or 0),
            "following_count": str(getattr(profile, "follows_count", 0) or 0),
            "posts_count": str(getattr(profile, "posts_count", 0) or 0),
        ...
        self.push_status(
            "DATA",
            f"Stats for @{target_handle}: {stats['followers_count']} followers, "
            f"{stats['following_count']} following, {stats['posts_count']} posts",
    ...
    def update_description(self, new_description: str) -> tuple[bool, str]:
        new_description = (new_description or "").strip()
        if not new_description:
            self.push_status("ERROR", "No description provided")
            return False, "No description provided"
        return self._start_thread(self._do_update_description, new_description)
    ...
    def get_user_stats(self, handle: Optional[str] = None) -> tuple[bool, str]:
        """Fetch followers, following, posts count, etc. in a background thread.
